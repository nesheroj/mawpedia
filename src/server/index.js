...
if (maxForks === 1 || cluster.isMaster) {
  ...
  cluster.on('exit', (worker, code, signal) => {
    console.warn(
      `[${packageInfo.name}] ⚠️ Server worker ${worker.process.pid} died! Restarting...`,
    );
  });

  console.info(
    `[${packageInfo.name} v${packageInfo.version}]  Server is starting on port ${PORT}.`,
  );
}
...
if (maxForks === 1 || !cluster.isMaster) {
  ...
  router.use(
    bodyParser({ enableTypes: ['text'], extendTypes: { text: ['text/plain'] } }),
    unpackRequest(),
    packResponse(),
  );
  ...
  app.listen(PORT, (err) => {
    ...
    console.info(
      `[${packageInfo.name} v${packageInfo.version}] 🐐  Server worker ${
        process.pid
      } is now serving.`,
    );
  });
